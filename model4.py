# 필요한 패키지 설치
!pip install pyngrok fastapi uvicorn torch diffusers transformers google-generativeai pillow nest-asyncio xformers googletrans
...
def generate_enhanced_prompt(entry: DiaryEntry) -> str:
    # 한국어를 영어로 번역
    translated_crop = "apple" if entry.crop == "사과" else translator.translate(entry.crop, dest='en').text
    translated_weather = translator.translate(entry.weather, dest='en').text
    translated_issues = translator.translate(entry.issues, dest='en').text
    translated_work = translator.translate(entry.work, dest='en').text
